...
# A Apollo exige a chave no cabeçalho x-api-key (o formato antigo de enviar
# 'api_key' dentro do corpo da requisição foi descontinuado e agora é rejeitado
...
def buscar_empresas_por_regiao(cidades, setor_palavras_chave=None, limit=50, faixas_funcionarios=None):
    ...
    for cidade in cidades:
        ...
        for pagina in range(1, max_paginas_por_cidade + 1):
            ...
            try:
                response = requests.post(url, json=payload, headers=APOLLO_HEADERS, timeout=15)
            except requests.exceptions.RequestException as e:
            ...
            if response.status_code != 200:
                try:
                    ...
                    detalhe = ''
                erros.append(f"{cidade}: Apollo retornou status HTTP {response.status_code}" + (f" — {detalhe}" if detalhe else ""))
                break
...
def buscar_perfil_apollo(nome, empresa):
    ...
    try:
        ...
        response = requests.post(url, json=payload, headers=APOLLO_HEADERS, timeout=10)
        if response.status_code == 200:
            data = response.json()
